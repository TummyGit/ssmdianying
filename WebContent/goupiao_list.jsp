...
<div class="form-search">
    <form name="form1" id="formSearch" method="get" action="">
        搜索:
        分类：<select data-value="${param.fenlei}" id="fenlei" name="fenlei" class="class_fenlei15">
        ...
        电影名称：<input type="text" style="" name="dianyingmingcheng" value="${param.dianyingmingcheng}"/> 开场时间：<input
            ...
            onfocus="WdatePicker({dateFmt:'HH:mm',lang:'zh-cn'})">-<input type="text" name="kaichangshijian_end"
                                                                          onfocus="WdatePicker({dateFmt:'HH:mm',lang:'zh-cn'})">
        选座：<input type="text" style="" name="xuanzuo" value="${param.xuanzuo}"/>
        ...
        <table width="100%" border="1" class="table table-list table-bordered table-hover">
            ...
            <c:forEach items="${list}" var="map">
                <tr>
                    ...
                    <td><ssm:sql var="mapyingpianfenlei15"
                                 type="find">SELECT fenleimingcheng FROM yingpianfenlei where id=${map.fenlei}</ssm:sql>${mapyingpianfenlei15.fenleimingcheng} </td>
                    <td> ${map.dianyingmingcheng} </td>
                    <td> ${map.piaojia} </td>
                    <td><ssm:sql var="mapfangyingting16"
                                 type="find">SELECT yingtingmingcheng FROM fangyingting where id=${map.fangyingting}</ssm:sql>${mapfangyingting16.yingtingmingcheng} </td>
