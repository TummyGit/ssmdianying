...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF" style="border-collapse:collapse">
  ...
  <%
   new CommDAO().delete(request,"liuyanban");
	...
	//out.print(sql);
	List<HashMap> list = Query.make("liuyanban").order("id desc").page(10); //PageManager.getPages(url,10,sql, request );
	int i=0;
  ...
  <tr>
    ...
    <td><img src="<%= map.get("zhaopian") %>"/></td>
    <td><%= map.get("QQ") %></td>
    ...
    <td><%= map.get("huifu") %></td>
    <td>
    <a href="liuyanban_list.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')">删除</a>
    <a href="hf.do?id=<%=map.get("id")%>">回复</a>
    </td>
  </tr>
  	<%
  }
